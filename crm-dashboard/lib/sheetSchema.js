...
//   1) 관리자(보험사업부) 시트 — "오프매장 전환DB_replit" 탭. 고객 설문 페이지가 여기 A~P열에
//      직접 기록하고, 보험사업부가 Q열 이후에 상담 진행상황을 기록합니다.
//   2) 매장 시트 — "신청인원 현황" 탭. B~L열은 관리자 시트를 IMPORTRANGE로 그대로 가져온 값(읽기전용)이고,
//      M~O열(고객 선물 지급일자·키트 불출인원·특이사항)은 매장 직원이 직접 입력하며, 이 값이 다시
//      IMPORTRANGE로 관리자 시트 AG~AI열에 반영됩니다.
...
// "storeMirror"(매장 시트 값을 IMPORTRANGE로 가져온 것, 관리자 시트 쪽은 읽기전용이며 실제 수정은
// 매장 시트에 직접 해야 함) 네 가지로 분류해 씁니다.
...
// 이 앱에서 씁니다. B~L열은 관리자 시트를 IMPORTRANGE로 그대로 가져온 값이라 절대 쓰지 않습니다.
// timestamp(B열, 관리자 시트 A열과 동일한 값)로 두 시트의 같은 신청 건을 짝지어 찾습니다.

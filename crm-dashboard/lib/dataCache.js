// 로그인 직후 모든 탭의 데이터를 미리 받아두는 공용 캐시입니다.
// 브라우저 탭이 열려있는 동안(전체 새로고침 전까지) 메모리에 유지되므로,
// 사용자가 실제로 그 탭에 들어갔을 때 다시 불러올 필요 없이 바로 화면을 그릴 수 있습니다.
...
// 기존 캐시 데이터에 일부 필드만 덮어써서 갱신합니다(로컬 저장/추가 등 즉시 반영용).
...
// 같은 key로 동시에 여러 곳(예: 로그인 직후 프리페치 + 막 마운트된 페이지)에서 요청해도
// 실제 네트워크 요청은 한 번만 보내고 결과를 함께 사용합니다.
...
// 로그인 직후(또는 첫 인증 페이지 진입 시) 한 번만 호출됩니다. 모든 탭의 데이터를
// 백그라운드에서 미리 받아둬서 사용자가 실제로 그 탭에 들어갔을 때 로딩 없이 바로
// 화면이 보이게 합니다.
